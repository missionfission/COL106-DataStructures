...
{		int N = 257311;
	...
	findCombinations obj=new findCombinations();
	String s1;
	String s2;
    int []a;
    int length;
...
sol1=new ArrayList<> ();
obj.Combinations(sol1,length,i,mylist);
...
{	
	...
	obj.Combinations(sol1,length,i,mylist);
	obj.Combinations(sol2,length-i,j,mylist2);
...
private class findCombinations{
	List <ArrayList<Integer>>mylist;ArrayList<Integer> sol;int length,k;

	public void Combinations(ArrayList<Integer> s,int len,int l,List <ArrayList<Integer>> list)
	{			mylist=list;
				sol=s;
				length=len;
				k=l;
...
public void recursive(int n)
  { if (sol.size() == k) //stop condition for the recursion
